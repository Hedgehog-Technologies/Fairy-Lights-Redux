...
import org.hedgetech.fairylightsredux.jingle.JinglePlayer;
...
public final class HangingLightsConnection extends HangingFeatureConnection<Light<?>> {
    ...
    public boolean interact(final Player player, final Vec3 hit, final FeatureType featureType, final int feature, final ItemStack heldStack, final InteractionHand hand) {
        if (featureType == FEATURE && heldStack.is(RecipeDefs.LIGHTS)) {
            ...
            if (!ItemStack.matches(light, heldStack)) {
                ...
                ItemUtils.giveItemToPlayer(player, light);
                this.computeCatenary();
                this.world.playSound(null, hit.x, hit.y, hit.z, FLRSounds.get(SoundDefs.FEATURE_COLOR_CHANGE), SoundSource.BLOCKS, 1.0F, 1.0F);
                return true;
            }
        }
    ...
    private void updateNeighbors(final Fastener<?> fastener) {
        this.world.updateNeighbourForOutputSignal(fastener.getPos(), FLRBlocks.get(BlockDefs.FASTENER));
    }
    ...
    protected Light<?> createFeature(final int index, final Vec3 point, final float yaw, final float pitch) {
        ...
        LightVariant<?> variant = LightVariantUtil.get(lightData);
        if (variant == null) {
            variant = SimpleLightVariant.FAIRY_LIGHT;
        }
        return this.createLight(index, point, yaw, pitch, lightData, variant);
    }
    ...
    protected float getFeatureSpacing() {
        ...
        for (final ItemStack patternLightData : this.pattern) {
            LightVariant<?> variant = LightVariantUtil.get(patternLightData);
            if (variant == null) {
                variant = SimpleLightVariant.FAIRY_LIGHT;
            }
            final float lightSpacing = variant.getSpacing();
