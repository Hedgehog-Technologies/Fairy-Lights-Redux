...
public final class RandomArray {
    ...
    public float get(final int index) {
        return this.values[FLRMth.mod(index, this.values.length)];
    }

    public float get(final float t) {
        final int t0 = Mth.floor(FLRMth.mod(t, this.values.length));
        final int t1 = FLRMth.mod(t0 + 1, this.values.length);
