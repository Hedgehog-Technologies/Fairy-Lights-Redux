...
public final class Catenary implements Curve {
    ...
    public Curve lerp(final Curve curve, final float delta) {
        ...
        if (curve.getClass() != this.getClass()) return curve;
        Catenary other = (Catenary) curve;
        if (this.count > other.count) return other.lerp(this, 1.0F - delta);
        ...
        final float angle = FLRMth.lerpAngle(this.yaw, other.yaw, delta);
    ...
    public void visitPoints(final float spacing, final boolean center, final PointVisitor visitor) {
        ...
        int index = 0;
        final Curve.SegmentIterator it = this.iterator();
        while (it.next()) {
    ...
    public SegmentIterator iterator(final boolean inclusive) {
        return new CurveSegmentIterator<>(this, inclusive) {
            ...
            @Override
            protected float getPitch(final int index) {
    ...
    public static Catenary from(final Vec3 direction, final float verticalYaw, final float ropeLength) {
        final float endX = Mth.sqrt((float) (direction.x * direction.x + direction.z * direction.z));
        final float endY = (float) direction.y;
        final float angle = endX < 1e-3F ? endY < 0.0F ? verticalYaw + FLRMth.PI : verticalYaw : (float) Mth.atan2(direction.z, direction.x);
        final float vx = Mth.cos(angle);
        final float vz = Mth.sin(angle);
        if (direction.length() > 2.0F * Connection.MAX_LENGTH) {
    ...
    private static float lengthFunc(final CubicBezier bezier, final double length) {
        return bezier.eval(Mth.clamp((float) length / Connection.MAX_LENGTH, 0.0F, 1.0F)) * Connection.MAX_LENGTH;
    }
}
