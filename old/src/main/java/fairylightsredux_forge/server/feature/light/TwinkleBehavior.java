...
import org.hedgetech.fairylightsredux.server.datacomponent.TwinkleComponent;
...
public class TwinkleBehavior implements BrightnessLightBehavior {
    ...
    public float getBrightness(final float delta) {
        if (this.powered) {
            ...
            return x < 0.25F ? 1.0F - EASE_IN_OUT.eval(x / 0.25F) : EASE_IN_OUT.eval(FLRMth.transform(x, 0.25F, 1.0F, 0.0F, 1.0F));
        }
        return 0.0F;
    }
    ...
    public static boolean exists(final ItemStack stack) {
        TwinkleComponent cmp = stack.get(TwinkleComponent.TYPE);
        return cmp != null && cmp.hasTwinkle();
    }
}
